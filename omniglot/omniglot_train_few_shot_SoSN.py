...
def main():
    ...
    for episode in range(EPISODE):
        ...
        # calculate features
        support_features = feature_encoder(Variable(supports).cuda(GPU)) # 5x64*5*5
        support_features = support_features.view(CLASS_NUM,SUPPORT_NUM_PER_CLASS,FEATURE_DIM,25).sum(1) #size: CLASS_NUMx64x5x5
        query_features = feature_encoder(Variable(queries).cuda(GPU)).view(QUERY_NUM_PER_CLASS*CLASS_NUM,64,25) #size: QUERY_NUM_PER_CLASSx64x5x5
        ...
        if (episode)%2500 == 0:
          with torch.no_grad():
            ...
            if test_accuracy > best_accuracy:
                # save networks
                torch.save(feature_encoder.state_dict(),str(METHOD + "/omniglot_feature_encoder_" + str(CLASS_NUM) +"way_" + str(SUPPORT_NUM_PER_CLASS) +"shot.pkl"))
                torch.save(relation_network.state_dict(),str(METHOD + "/omniglot_similarity_network_"+ str(CLASS_NUM) +"way_" + str(SUPPORT_NUM_PER_CLASS) +"shot.pkl"))
