...
def main():
    ...
    feature_encoder = FeatureEncoder().apply(weights_init).cuda(GPU)
    relation_network = SimilarityNetwork(FEATURE_DIM,RELATION_DIM).apply(weights_init).cuda(GPU)
            ...
            for i in range(TEST_EPISODE):
              with torch.no_grad():
                ...
                for query_images,query_labels in query_dataloader:
                    ...
                    # calculate features
                    support_features = feature_encoder(Variable(support_images, volatile=True).cuda(GPU)).view(CLASS_NUM,64,19**2) # 5x64
                    query_features = feature_encoder(Variable(query_images, volatile=True).cuda(GPU)).view(num_per_class*CLASS_NUM,64,19**2) # 20x64
                    ...
                    rewards = [1 if predict_labels[j]==query_labels[j] else 0 for j in range(query_size)]
